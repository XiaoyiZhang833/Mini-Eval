...
_PATTERN_TRANSLATIONS = [
    ...
    (
        re.compile(r"^当前样本包含\s*(\d+)\s*个输出，请逐条标注（每行 3 个）。$"),
        lambda m: f"Current sample has {m.group(1)} outputs. Annotate each (3 per row).",
    ),
    (
        re.compile(r"^当前样本包含\s*(\d+)\s*个输出，请对每个输出分别标注（每行 3 个）。$"),
        lambda m: f"Current sample has {m.group(1)} outputs. Annotate each output separately (3 per row).",
    ),
    (
