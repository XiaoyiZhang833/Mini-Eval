...
def _load_archive_checkpoint(file_name: str) -> tuple:
    ...
    if eval_mode in ["manual", "multi_manual"]:
        ...
        option_pool.sort(key=lambda x: str(x))
        for k in option_pool_by_output:
            option_pool_by_output[k].sort(key=lambda x: str(x))
...
def _render_compact_metric_style() -> None:
    ...
    st.markdown(
        ...
        .st-key-clear_file_top_running button,
        .st-key-clear_file_top_completed button {
            border-color: #f6c2c7 !important;
...
def _bump_metrics_revision() -> None:
    """递增统计缓存版本，确保浏览编辑后指标会重算。"""
    st.session_state.metrics_revision = int(
        st.session_state.get("metrics_revision", 0) or 0
    ) + 1
...
def _collect_display_metrics_for_mode():
    ...
    mode = st.session_state.eval_mode
    snapshot = (mode, int(st.session_state.get("metrics_revision", 0) or 0))
    cached_snapshot = st.session_state.get("display_metrics_snapshot")
    cached_payload = st.session_state.get("display_metrics_payload")
    if cached_snapshot == snapshot and isinstance(cached_payload, dict):
    ...
    if not _has_active_browse_overrides():
        if mode == "direct":
            ...
            st.session_state.display_metrics_snapshot = snapshot
            st.session_state.display_metrics_payload = result
            return result
        if mode == "multi":
            metrics = compute_direct_metrics(st.session_state.multi_output_decisions)
            result = {
                "metrics": metrics,
...
def _build_export_snapshot(total_count: int, current_index: int) -> tuple:
    """构建轻量快照，用于判断导出缓存是否过期。"""
    records = st.session_state.get("records", [])
    first_id = ""
    last_id = ""
    if isinstance(records, list) and records:
        first_id = str(records[0].get("id", "")) if isinstance(records[0], dict) else ""
        last_item = records[-1]
        last_id = str(last_item.get("id", "")) if isinstance(last_item, dict) else ""

    cell_overrides = st.session_state.get("browse_cell_overrides", {})
    override_cell_count = 0
    if isinstance(cell_overrides, dict):
        for value in cell_overrides.values():
            if isinstance(value, dict):
                override_cell_count += len(value)

    return (
        str(st.session_state.get("eval_mode", "")),
        int(total_count),
        int(min(current_index, total_count)),
        len(st.session_state.get("direct_decisions", []) or []),
        len(st.session_state.get("manual_annotations", []) or []),
        len(st.session_state.get("multi_output_annotations", []) or []),
        len(st.session_state.get("multi_output_manual_annotations", []) or []),
        bool(st.session_state.get("browse_rows_dirty", False)),
        override_cell_count,
...
def _build_current_export_excel(total_count: int, current_index: int) -> tuple:
    """按当前状态构建导出 Excel 内容。"""
    mode = st.session_state.eval_mode
    ...
    try:
        ...
        return (
            ...
            f"evaluation_{mode}_current_{timestamp}.xlsx",
        )
    except Exception:
        return (None, "")
...
def _render_top_export_current_button(
    ...
    """在页面右上方渲染当前结果导出按钮。"""
    snapshot = _build_export_snapshot(total_count=total_count, current_index=current_index)
    cached_snapshot = st.session_state.get("top_export_snapshot")
    cached_excel = st.session_state.get("top_export_excel_content")
    cached_file_name = str(st.session_state.get("top_export_file_name", "")).strip()
    if st.button(
        "导出当前结果",
        type=button_type,
        ...
        if cached_excel and cached_snapshot == snapshot:
            excel_content = cached_excel
            file_name = cached_file_name or "evaluation_current.xlsx"
        else:
            excel_content, file_name = _build_current_export_excel(
                total_count=total_count,
                current_index=current_index,
            )
            if excel_content is None:
                st.session_state.top_export_snapshot = None
                st.session_state.top_export_excel_content = None
                st.session_state.top_export_file_name = ""
                st.error("导出文件生成失败，请稍后重试。")
                return

            st.session_state.top_export_snapshot = snapshot
            st.session_state.top_export_excel_content = excel_content
            st.session_state.top_export_file_name = file_name

        payload_b64 = base64.b64encode(excel_content).decode("ascii")
        safe_name = html.escape(str(file_name or "evaluation_current.xlsx"), quote=True)
        components.html(
            f"""
            <script>
            (function() {{
                const fileName = {safe_name!r};
                const href = "data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{payload_b64}";
...
def _render_top_action_bar(
    ...
    with st.container(border=True):
        left_col, right_col = st.columns([1.7, 8.3], gap="small")
        ...
        with right_col:
            (
                ...
                [4.2, 0.9, 0.14, 0.9, 0.14, 1.25, 0.14, 1.15, 0.14, 1.55],
                gap="small",
            )
        ...
        with progress_center_col:
            st.markdown(
                f"<div class='top-progress-meta'>{t(f'当前第 {progress_current} 条 / 共 {total_count} 条')}</div>",
                unsafe_allow_html=True,
            )
            st.progress(progress_ratio)

        st.markdown("<div class='top-action-divider'></div>", unsafe_allow_html=True)
...
def _render_empty_state(example_file_path: Path) -> None:
    ...
    st.markdown("#### 字段映射")
    source_name = st.session_state.import_source_name or t("当前数据")
    st.caption(
...
def _render_manual_mode(current_item: dict) -> None:
    ...
    if input_mode == "选择":
        st.caption("可从已有结果集合中选择；新增/删除请在工具面板点击“管理选择项”。")
        ...
        if option_pool:
            ...
            st.warning("当前结果集合为空，请在工具面板先新增选择项。")
        return
...
def _render_manual_save_action(current_item: dict) -> None:
    ...
    if st.button("保存并下一条", type="primary", use_container_width=True):
        input_mode = st.session_state.get(
            f"manual_input_mode_{current_item['id']}", "手动输入"
        )
        if input_mode in ("选择", "Select"):
...
def _render_multi_manual_mode(current_item: dict) -> None:
    ...
    hide_llm_output = bool(st.session_state.get("hide_llm_output", False))
    if hide_llm_output:
        st.markdown(
            f'<div class="annotation-hint">{t("LLM 输出已隐藏。请根据每个输入框上方的 Output 标签填写对应结果。")}</div>',
            unsafe_allow_html=True,
        )
        st.caption("当前为人工输入结果模式，LLM 输出已隐藏。")
    else:
        ...
        for col_offset in range(cols_per_row):
            ...
            with row_cols[col_offset]:
                if not hide_llm_output:
                    ...
                    st.caption(f"{t('Output')}: {field_name}")
                ...
                if input_mode == "选择":
                    ...
                    if option_pool:
                        ...
                        st.warning(
                            f"{field_name} 的结果集合为空，请在工具面板先新增选择项。"
                        )
                else:
                    st.text_area(
                        f"{field_name} 人工输入结果",
...
def render_option_pool_manager_tool(current_item: dict | None) -> None:
    """在工具面板渲染统一的选择项管理入口。"""
    mode = str(st.session_state.get("eval_mode", ""))
    is_supported_mode = mode in ["manual", "multi_manual"]
    has_records = bool(st.session_state.get("records"))

    if st.button(
        "管理选择项",
        width="stretch",
        disabled=(not has_records or not is_supported_mode),
        key="tool_open_option_pool_manager",
    ):
        st.session_state.show_option_pool_manager_dialog = True
        st.rerun()

    if not bool(st.session_state.get("show_option_pool_manager_dialog", False)):
        return
    ...
    def _close_dialog() -> None:
        st.session_state.show_option_pool_manager_dialog = False
        st.rerun()

    def _manager_body() -> None:
        if not has_records:
            st.warning("当前没有可管理的数据。")
            if st.button("关闭", width="stretch", key="tool_option_pool_close_empty"):
                _close_dialog()
            return

        if mode == "manual":
            output_choices = ["output_1"]
            target_output_key = "output_1"
            target_out_idx = 1
        else:
            output_labels = []
            if isinstance(current_item, dict):
                ...
                output_list = current_item.get(
                    "output_list",
                    split_output_values(current_item.get("output", "")),
                )
                output_count = len(output_list)
            else:
                records = st.session_state.get("records", [])
                first = records[0] if isinstance(records, list) and records else {}
                output_labels = first.get("output_labels", []) if isinstance(first, dict) else []
                output_list = first.get("output_list", []) if isinstance(first, dict) else []
                output_count = len(output_list)

            output_choices = []
            for idx in range(1, max(1, output_count) + 1):
                label = (
                    str(output_labels[idx - 1]).strip()
                    if idx - 1 < len(output_labels) and str(output_labels[idx - 1]).strip()
                    else f"output_{idx}"
                )
                output_choices.append(f"{label} (#{idx})")

            selected_choice = st.selectbox(
                "选择要管理的 Output",
                options=output_choices,
                key="tool_option_pool_target_output",
            )
            target_out_idx = int(re.search(r"#(\d+)\)$", selected_choice).group(1))
            target_output_key = selected_choice

        if mode == "manual":
            pool = list(st.session_state.get("manual_option_pool", []))
        else:
            pool_by_output = dict(st.session_state.get("manual_option_pool_by_output", {}))
            pool = list(pool_by_output.get(target_out_idx, []))

        st.caption(f"当前管理：{target_output_key}")
        new_option = st.text_input(
            "新增选择项",
            placeholder="输入要新增到集合中的结果文本",
            key="tool_option_pool_new_text",
        )

        add_col, close_col = st.columns(2)
        with add_col:
            if st.button("加入选择项", width="stretch", key="tool_option_pool_add"):
                candidate = str(new_option).strip()
                if not candidate:
                    st.error("新增选择项不能为空。")
                elif candidate in pool:
                    st.warning("该选择项已存在，无需重复添加。")
                else:
                    pool.append(candidate)
                    if mode == "manual":
                        st.session_state.manual_option_pool = pool
                    else:
                        pool_by_output[target_out_idx] = pool
                        st.session_state.manual_option_pool_by_output = pool_by_output
                    st.rerun()

        with close_col:
            if st.button("关闭", width="stretch", key="tool_option_pool_close"):
                _close_dialog()
        ...
        delete_option = st.selectbox(
            "删除选择项",
            options=pool,
            key="tool_option_pool_delete_select",
        )
        if st.button("删除选择项", width="stretch", key="tool_option_pool_delete"):
            if delete_option in pool:
                pool.remove(delete_option)
                if mode == "manual":
                    st.session_state.manual_option_pool = pool
                else:
                    pool_by_output[target_out_idx] = pool
    ...
    try:
        ...
        def _option_pool_manager_dialog() -> None:
            _manager_body()
...
def render_evaluation_panel(example_file_path: Path):
    ...
    col_left, col_right = st.columns([5, 5], gap="large", vertical_alignment="top")

    with col_right:
        with st.container(border=True):
            mode = str(st.session_state.get("eval_mode", ""))
            header_col, toggle_col = st.columns([6, 4], vertical_alignment="center")
            with header_col:
                st.markdown("##### 🎯 标注操作")
            if mode in ["manual", "multi_manual"]:
                with toggle_col:
                    show_llm_output = st.toggle(
                        "显示 LLM 的结果",
                        value=not st.session_state.hide_llm_output,
                        key="annotation_show_llm_output",
                    )
                st.session_state.hide_llm_output = not bool(show_llm_output)
            else:
            ...
            if st.session_state.eval_mode == "manual":
                ...
                st.radio(
                    ...
                    horizontal=True,
                    label_visibility="collapsed",
                    key=f"direct_choice_{current_item['id']}",
                )
                st.divider()
                if st.button("保存并下一条", type="primary", use_container_width=True):
                    choice = st.session_state.get(
                        f"direct_choice_{current_item['id']}"
                    )
                    if choice not in ["采纳", "拒绝"]:
                        st.error("请先完成标注。")
                        return
                    record_direct_decision(is_accept=(choice == "采纳"))
                    go_next_item()
                    st.rerun()
